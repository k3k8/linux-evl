...
static bool
irqentry_syncstage(irqentry_state_t state) /* inband, hard irqs off */
{
	/*
...
static inline bool irqexit_may_preempt_schedule(irqentry_state_t state,
						struct pt_regs *regs)
{
...
{
	if (!irqexit_may_preempt_schedule(state, regs) || state.exit_rcu)
		return;
