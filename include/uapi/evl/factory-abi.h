...
/*
 * Valid flags for the clone operation. The core only uses bits 16-31,
 * the rest is available to libevl.
 */
...
/*
 * Element is owned by the process creating it. Such element has no
 * file binding, which implies that only fundle-based operations can
 * be requested for it. Such element is private by design (i.e. no
 * entry under /dev/evl), but can still be shared between
 * processes. However, its lifetime is bound to its creator.
 */
#define __EVL_CLONE_OWNED	(1 << 30)

/*
 * Element is created by the core or some global kernel context by
 * extension, as such it may not be owned (__EVL_CLONE_OWNED must be
 * clear if __EVL_CLONE_CORE is set).  This modifier is used as a
 * discriminator to skip user-oriented setup operations.
 */
#define __EVL_CLONE_CORE	(1 << 31)

/*
 *  All the valid clone flags as far as user is concerned.
 */
#define EVL_CLONE_MASK		(((__u32)-1 << 16) & ~__EVL_CLONE_CORE)
