...
bool handle_oob_irq(struct irq_desc *desc)
{
	 ...
	 */
	if (WARN_ON_ONCE(irq_pipeline_debug() && irq_desc_is_chained(desc)))
		return false;
...
struct irq_stage_data *handle_irq_pipelined_prepare(struct pt_regs *regs)
{
	 ...
	 * sides (e.g. generic_handle_domain_irq() needs this to
	 * resolve IRQ mappings).
	 */
	ct_nmi_enter();
...
void __init irq_pipeline_init(void)
{
	 ...
	 * enough system-wide.
	 *
	 * CAVEAT: __irq_domain_create() is a bit fragile, max_irq
	 * must not translate to a negative value when coerced to a
	 * signed int, otherwise the domain creation would fail.
	 */
	synthetic_irq_domain = irq_domain_create_nomap(NULL, 1024,
						    &sirq_domain_ops,
						    NULL);
	inband_work_sirq = irq_create_direct_mapping(synthetic_irq_domain);
	ret = request_percpu_irq_affinity_flags(inband_work_sirq, inband_work_interrupt,
						IRQF_NO_THREAD, "in-band irq work",
						NULL, &pipeline_percpu_data);
	WARN_ON(ret);
	...
	if (num_possible_cpus() > 1 &&
		request_percpu_irq_affinity_flags(CALL_FUNCTION_OOB_IPI,
