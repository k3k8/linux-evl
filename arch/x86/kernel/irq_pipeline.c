...
/*
 * This transparent trampoline call works around a warning from
 * objtool ("__sysvec_reboot() missing __noreturn in .c/.h or
 * NORETURN() in noreturns.h") without opening a can of worms fiddling
 * with run_sysvec_on_irqstack_cond() typechecking.
 */
static void __do_reboot(struct pt_regs *regs)
{
...
static void do_sysvec_inband(struct irq_desc *desc, struct pt_regs *regs)
{
	 ...
	 *
	 * 	arch_handle_irq(irq)
	 *		generic_pipeline_irq_desc(desc)
	 *			handle_apic_irq(irq)
	 *				handle_oob_irq(irq)
	 *				[...irq_post_inband...]
	 *
	...
	case RESCHEDULE_VECTOR:
		run_sysvec_on_irqstack_cond(__sysvec_reschedule_ipi, regs);
		break;
	case CALL_FUNCTION_VECTOR:
		run_sysvec_on_irqstack_cond(__sysvec_call_function, regs);
		break;
	case CALL_FUNCTION_SINGLE_VECTOR:
		run_sysvec_on_irqstack_cond(__sysvec_call_function_single, regs);
		break;
	case REBOOT_VECTOR:
		run_sysvec_on_irqstack_cond(__do_reboot, regs);
		break;
#endif
	case X86_PLATFORM_IPI_VECTOR:
		run_sysvec_on_irqstack_cond(__sysvec_x86_platform_ipi, regs);
		break;
	...
	case POSTED_INTR_VECTOR:
		run_sysvec_on_irqstack_cond(__sysvec_kvm_posted_intr_ipi, regs);
		break;
	case POSTED_INTR_WAKEUP_VECTOR:
		run_sysvec_on_irqstack_cond(__sysvec_kvm_posted_intr_wakeup_ipi, regs);
		break;
...
{
	sipic_domain = irq_domain_create_simple(NULL, NR_APIC_VECTORS,
					     FIRST_SYSTEM_IRQ,
